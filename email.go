...
func (e emailService) SendPasswordResetEmail(recipient Recipient) error {
	// Compose email
	message := mail.NewV3Mail()
	message.SetFrom(AdminSender)
	message.SetTemplateID(PasswordResetTemplateID)
	...
	p.SetDynamicTemplateData("VerificationLink", "")
...
func (e emailService) SendVerificationEmail(code string, recipient Recipient) error {
	...
	message := mail.NewV3Mail()
	message.SetFrom(AdminSender)
	message.SetTemplateID(VerificationTemplateID)
	...
	// Set verification code
	p.SetDynamicTemplateData("VerificationLink", code)
	message.AddPersonalizations(p)
